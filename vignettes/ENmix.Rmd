...
<li>`nmode()`:   Identify gap probes</li>
<li>`dupicc()`:     Calculate ICC with data for duplicates</li>
...
<li>`estimateCellProp()`:    Estimate cell type proportions</li>
<li>`methyAge()`:     Calculate methylation age</li>
<li>`predSex()`:      Estimate sample sex</li>
<li>`ctrlsva()`:      Derive surrogate variables to control for experimental confoundings using non-negative control probes</li>
...
the number of probes per array. However, we recommand to provide correct manifest file (.csv format)
