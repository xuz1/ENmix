...
\date{Modified: October 3 2020.  Compiled: \today}
\begin{document}
\maketitle
...
correction and quanlity control in a single step. 
...
#if qc info object is provided, the low quality or outlier samples and probes 
...
#if set detPtype="negative", recommand to set depPthre to at least 10E-6
...
#beta matrix before rm.outlier() step should be used for this purpose
...
#Non-negative control surrogate variables, which can be used in downstream 
#association analysis to control for batch effects. 
csva<-ctrlsva(rgSet)
...
#ICC analysis can be performed to study measurement reliability if have duplicates, see manual
#dupicc()
...
integrated, and having smaller data size (about 1/3 smaller). User can also 
...
library(ENmix)
rgSet <- readidat(path = "directory path for idat files", recursive = TRUE)
@
...
conversion probes can be extracted or generated using the 
...
These small percentage of data points can have a large impact on association
statistical tests for individual CpGs. 
Function \Rfunction{rm.outlier} can be used to filter out these
...
b2=rm.outlier(beta,qcscore=qc)
#filter out low quality and outlier values, remove rows 
#and columns with too many missing values
b3=rm.outlier(beta,qcscore=qc,rmcr=TRUE)
#filter out low quality and outlier values, remove rows and
# columns with too many missing values, and then do imputation
b3=rm.outlier(beta,qcscore=qc,rmcr=TRUE,impute=TRUE)
@
...
internal control probes. These variables can be modeled as covariables in 
...
linear regression with each specified variables, such as batch or phnotype
...
components were ordered according to the percentage of variace they explained, 
from large to small. 
...
\section{Multimodal CpGs and gap probes}
...
Whole blood samples are often used in the EWAS, however, whole blood has 
...
there are 6 publically deposited reference datasets: 
...
\Rfunction{methDataSet},\Rfunction{MethylSet},\Rfunction{RGChannelSet} 
or methylation beta value matrix.


\section{Methylation age estimation}

Methylation age can reflect a person's biological age, which is more
...
\Rfunction{methyAge}. They are Horvath, Hannum and PhenoAge. 
...
\section{Differentially methylated regions}
...
CpGs can potentially increase association signal. Function \Rfunction{ipdmr} and 
\Rfunction{combp} can utilize P values from EWAS to identify 
differentialy methylated regions (DMR). 
...
Several studies showed that a large percentage of CpGs on Illumina array
have poor reliability, which means low correlation between replicate meansures 
of a same set of samples. Measurement reliability can be reflected by intraclass
 correlation calculated in methylation data of duplicates. Function 
\Rfunction{dupicc} can be used for the purpose. 
...
maximum Likelihood Estimate (MLE) of 5mC and 5hmC using sequencing/array data from
 paired experiments.
