...
\date{Modified: August 31, 2015.  Compiled: \today}
\begin{document}
\maketitle
...
conversion probes can be
 ...
 to identify low quality data points. Samples or probes with large
...
(arguments: \textit{samplethre, CpGthre and bisulthre}). 
Users can also specify a list of samples or CpGs to
be filtered out by using options \textit{outid} and \textit{outCpG}.
 Frequency polygon plot for total intensity (methylated intensity 
+ unmethylated intensity) and methylation beta values before and 
after filtering can be generated using argument \textit{plot=TRUE}.
