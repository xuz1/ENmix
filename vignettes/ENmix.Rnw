...
\date{Modified: March 31, 2015.  Compiled: \today}
\begin{document}
\maketitle
...
HumanMethylation450 BeadChip, Under review.
  \item Function \Rfunction{normalization}:  \\
...
Function \Rfunction{bgcorrect} incorporated a model based background
...
Function \Rfunction{normalization} can be used to perform quantile 
normalization on methylation intensity value

<<normalization, eval=FALSE>>=
mdat<-normalization(mdat, method="quantile1")
@
...
should be excluded from DNA methylation analysis. Although researchers
 have often excluded CpGs based on SNP annotation information, we found
...
From the following summary table we can see this approach can correctly
 ...
 in both populations and could be kept in data analysis with exclusion of
 a few outliers. For CpGs with any SNP at CpG target site, more than 80\% 
