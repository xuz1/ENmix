...
\date{Modified: February 9, 2016.  Compiled: \today}
\begin{document}
\maketitle

\section{Introduction}

Illumina HumanMethylation450 and MethylationEPIC BeadChip array measurements have
intrinsic levels of background noise that degrade methylation measurement.
The \Rpackage{ENmix} package provides an efficient data pre-processing tool designed
to reduce background noise and improve signal for DNA methylation estimation.
The package utilizes a novel model-based background correction method, \textit{ENmix},
that significantly improve accuracy and reproducibility of methylation
measures. The data structure used by the \Rpackage{ENmix} package is compatible with
...
In addition the \Rpackage{ENmix} package has selectable complementary functions for
...
conversion probes can be
 ...
 \Robject{RGChannelSetExtended}. According default or user specified 
quality score thresholds, the \Rfunction{QCinfo} can also identify and 
export a list of low quality samples and CpG probes. Outlier samples in 
total intensity or beta value distribution were often excluded before
further analysis. Such samples were tricky to be identified, by default 
the argument outlier=TRUE will trigger the function to identify these 
outlier samples automatically. Quality score figures from
...
Outlier and low quality data values can have large impact on association
...
Function \Rfunction{preprocessENmix} incorporats a model based background
...
Users can also specify a list of poor performance CpGs to be excluded
 before background correction using argument \textit{exCpG}.
...
\section{Parallel computing wrappers}
DNA methylation analysis is computationally intensive. To take advantage 
of the widely available multi-core computation resources, we provided a few 
parallel computing wrappers to speed up the analysis. 

\subsection{Probe type bias correction}
Majority of probes on Illumina 450K and EPIC BeadChips are type II probes. Although 
...
The \Rfunction{BMIQ} function in R package \Rpackage{wateRmelon} can also be used to 
...
#if provide qc info, the low quality samples and probes will be excluded before background correction
mdat<-preprocessENmix(rgSet, bgParaEst="oob", dyeCorr=TRUE, QCinfo=qc, nCores=6)
#low quality samples and probes can also be excluded after background correction using QCfilter.
#mdat <- QCfilter(mdat,qcinfo=qc,outlier=TRUE)
...
\subsection{Example 2: A more elaborated example}
<<example, eval=FALSE>>=
library(ENmix)
#read in data
sheet <- read.450k.sheet(file.path(find.package("minfiData"),
...
correction method for Illumina Methylation BeadChip, in review
