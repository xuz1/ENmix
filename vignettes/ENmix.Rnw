...
rgSet <- readidat(path = path,recursive = TRUE)
sheet <- read.metharray.sheet(file.path(find.package("minfiData"),"extdata"),
    pattern = "csv$") rownames(sheet)=paste(sheet$Slide,sheet$Array,sep="_")
colData(rgSet)=as(sheet[colnames(rgSet),],"DataFrame")
#control plots
...
Alternatively, the \Rfunction{BMIQ} function in R package \Rpackage{wateRmelon}
 can also be used to reduce probe type bias. However the funtion is computation 
intensive and can take very long time for large dataset. Therefore we here 
provided a multi-core parallel computing 
