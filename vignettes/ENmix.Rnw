...
mraw<-QCfilter(mraw, qcinfo=qc, samplethre = 0.01, CpGthre = 0.05
    ,bisulthre=5000,plot=TRUE, outid=NULL, outCpG=outCpG)
cglist=rownames(mraw)
idlist=colnames(mraw)
exCpG=rownames(beta)[!(rownames(beta) %in% cglist)]

#background correction
mdat<-preprocessENmix(rgSet, bgParaEst="oob", dyeCorr=TRUE, exCpG=exCpG, nCores=6)
mdat=mdat[,colnames(mdat) %in% idlist]
