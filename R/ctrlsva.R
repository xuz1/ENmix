...
ctrlsva<-function(rgSet,percvar=0.9,npc=1,flag=1)
{
    ...
    if(percvar<0 | percvar>1){stop("Percentage of variation threshold needs to be between 0 and 1\n")}
...
#PCA of control-probe intensities
    ...
    cat(npc," surrogate variables explain ",sum(perc[1:npc])*100, "% of data variation\n")
    ctrlsva
}
