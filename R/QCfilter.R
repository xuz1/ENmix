QCfilter <-function(mdat,qcinfo=NULL,detPthre=0.000001,nbthre=3,samplethre=0.05,CpGthre=0.05,
...
{    
    ...
    if(is.null(qcinfo)){stop("ERROR: Please provide QC information from function QCinfo'")}
    if(outlier & is.null(qcinfo$outlier_sample)){
       stop("ERROR: No outlier sample information, please set outlier=FALSE\n")}
    detP=qcinfo$detP
    nbead=qcinfo$nbead
    bisul=qcinfo$bisul
    outlier_sample=qcinfo$outlier_sample
    if(sum(!(colnames(mdat) %in% colnames(detP)))>0){stop("ERROR: some samples do not have QC information")}
    if(sum(!(rownames(mdat) %in% rownames(detP)))>0){stop("ERROR: some CpGs do not have QC information")}
