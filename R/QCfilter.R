...
function(mdat,qcinfo,detPthre=0.05,nbthre=3,samplethre=0.01,CpGthre=0.05,
         bisulthre=1,plot=TRUE, outid=NULL, outCpG=NULL)
{
    ...
    if(plot)
    {
        cat("Ploting freqpolygon_total_intensity_beforeQC.jpg ...")
        mu <- assayData(mdat)$Meth+assayData(mdat)$Unmeth
        jpeg(filename="freqpolygon_total_intensity_beforeQC.jpg",width=1000,
             height=500,quality = 100)

        multifreqpoly(mu,cex.lab=1.4,cex.axis=1.5,
                cex.main=1.5,main="Frequency polygon plot of total intensity before QC",
                xlab="Total intensity")
        dev.off()
        cat("Done\n")
        cat("Ploting freqpolygon_total_beta_beforeQC.jpg ...")
        beta <- getBeta(mdat, "Illumina")
        jpeg(filename="freqpolygon_total_beta_beforeQC.jpg",width=1000,
             height=500,quality = 100)
        multifreqpoly(beta,cex.lab=1.4,cex.axis=1.5,
                cex.main=1.5,main="Frequency polygon plot of Beta value before QC",
    ...
    ##Frequency polygon plot after QC
    if(plot)
    {
        cat("freqpolygon_total_intensity_afterQC.jpg ...")
        mu <- assayData(mdat)$Meth+assayData(mdat)$Unmeth
        jpeg(filename="freqpolygon_total_intensity_afterQC.jpg",width=1000,
             height=500,quality = 100)
        multifreqpoly(mu,cex.lab=1.4,cex.axis=1.5,
                cex.main=1.5,main="Frequency polygon plot of total intensity after QC",
                xlab="Total intensity")
        dev.off()
        cat("Done\n")
        
        cat("freqpolygon_total_beta_afterQC.jpg ...")
        beta <- getBeta(mdat, "Illumina")
        jpeg(filename="freqpolygon_total_beta_afterQC.jpg",width=1000,
             height=500,quality = 100)
        multifreqpoly(beta,cex.lab=1.4,cex.axis=1.5,
                cex.main=1.5,main="Frequency polygon plot of Beta value after QC",
                xlab="Methylation beta value")
        dev.off()
        cat("Done\n")
    }
