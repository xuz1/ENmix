...
plotp<-function(jpgfile,p,yaxis,xmax,title)
{
    ...
    plot(1,xlim=c(0,xmax),ylim=c(0,length(yaxis)+1),type="n",bty="n",axes=FALSE,
    xlab="Principal Component",ylab="",main=title)
    ...
    legend("topright",c("<0.05","<0.01","<10E-5","<10E-10"),col=c("pink","orange","red","darkred"),
...
pcrplot<-function(beta,cov,npc=50)
{
    ...
    cat("Top ",npc," principal components can explain ", prop, "% of data variation","\n")
    p <- lmmatrix(svd$x[,1:npc],cov)
    yaxis <- colnames(p)
    plotp("pcr_diag.jpg",p,yaxis,npc,title="Principal Component Regression Analysis")
