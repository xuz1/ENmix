relic <- function(mdat,at_red=NULL,cg_grn=NULL)
{
    ...
    if(is.null(at_red) | is.null(cg_grn))
        {stop("Internal control intensity matrics at_red and cg_grn must be provided\n")}

    # make sure the order is right
    name_grn <- gsub("G","A",gsub("C","T",rownames(cg_grn)))
    common=intersect(name_grn,rownames(at_red))
    if(length(common)<50)
       {stop("Please check Internal control intensity matrics at_red and cg_grn.\n
        Please assign internal control ExtendedType as row name for matrics
              at_red and cg_grn\n")}
