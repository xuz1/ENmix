...
oxBS.MLE<-function(beta.BS,beta.oxBS,N.BS,N.oxBS){
    if (!four.identical(rownames(beta.BS),rownames(beta.oxBS),rownames(N.BS),rownames(N.oxBS))){
        stop("Row names are not consistent!")
    }
    else if (!four.identical(colnames(beta.BS),colnames(beta.oxBS),colnames(N.BS),colnames(N.oxBS))){
        stop("Column names are not consistent!")
    }
    else {
        ...
        rownames(beta.5mC)<-rownames(beta.BS); rownames(beta.5hmC)<-rownames(beta.BS);
        colnames(beta.5mC)<-colnames(beta.BS); colnames(beta.5hmC)<-colnames(beta.BS);
